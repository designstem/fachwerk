...
new Vue({
  ...
  data: () => ({ value: 0, value2: 0.5, value3: 0, count: 0 }),
  methods: { cx, cy },
  template: `
    <div>
    <input type="range" v-model="value" step="0.01" max="4" />
    <input type="range" v-model="value2" step="0.01" max="4" />
    <input type="range" v-model="count" step="1" max="12" />
    <Renderer :size="{ w: 600, h: 400 }">
    <Scene>
      <Camera :position="{ z: 3 }" />
      <Mesh :rotation="{x: value2, y: value, z: 0}">
        <template
          v-for="(a,j) in Array.from({ length: 13 }).map((_,i) => i + 3)"
          v-if="count == j"
        >
          <Triangle
            v-for="(c,i) in a"
